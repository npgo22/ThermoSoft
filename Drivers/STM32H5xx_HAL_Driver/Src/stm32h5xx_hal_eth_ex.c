...
#define ETH_MACRXVLAN_MASK                                                                         \
  (ETH_MACVTR_EIVLRXS | ETH_MACVTR_EIVLS | ETH_MACVTR_ERIVLT | ETH_MACVTR_EDVLP |                  \
   ETH_MACVTR_VTHM | ETH_MACVTR_EVLRXS | ETH_MACVTR_EVLS | ETH_MACVTR_DOVLTC | ETH_MACVTR_ERSVLM | \
                                            ...
                                            ETH_RxVLANConfigTypeDef *pVlanConfig)
{
  ...
  pVlanConfig->InnerVLANTag =
      ((READ_BIT(heth->Instance->MACVTR, ETH_MACVTR_ERIVLT) >> 27) == 0U) ? DISABLE : ENABLE;
                                            ...
                                            ETH_RxVLANConfigTypeDef *pVlanConfig)
{
  ...
  MODIFY_REG(heth->Instance->MACVTR, ETH_MACRXVLAN_MASK,
             (((uint32_t) pVlanConfig->InnerVLANTagInStatus << 31) |
              ...
              pVlanConfig->VLANTypeCheck | ((uint32_t) pVlanConfig->VLANTagInverceMatch << 17)));
                                            ...
                                            ETH_TxVLANConfigTypeDef *pVlanConfig)
{
  ...
  if (VLANTag == ETH_INNER_TX_VLANTAG) {
    ...
    pVlanConfig->SVLANType =
        ((READ_BIT(heth->Instance->MACIVIR, ETH_MACVIR_CSVL) >> 19) == 0U) ? DISABLE : ENABLE;
    ...
    pVlanConfig->SVLANType =
        ((READ_BIT(heth->Instance->MACVIR, ETH_MACVIR_CSVL) >> 19) == 0U) ? DISABLE : ENABLE;
    pVlanConfig->VLANTagControl =
