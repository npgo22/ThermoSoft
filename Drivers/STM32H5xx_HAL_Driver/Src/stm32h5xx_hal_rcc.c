...
HAL_StatusTypeDef HAL_RCC_DeInit(void)
{
  ...
  CLEAR_BIT(RCC->CR, RCC_CR_CSION | RCC_CR_CSIKERON | RCC_CR_HSECSSON | RCC_CR_HSIKERON |
                         RCC_CR_HSI48ON | RCC_CR_HSEON);
  ...
  if ((pOscInitStruct->PLL.PLLState) != RCC_PLL_NONE) {
    ...
    if (temp_sysclksrc != RCC_SYSCLKSOURCE_STATUS_PLLCLK) {
      if ((pOscInitStruct->PLL.PLLState) == RCC_PLL_ON) {
        ...
        __HAL_RCC_PLL1_CONFIG(pOscInitStruct->PLL.PLLSource, pOscInitStruct->PLL.PLLM,
      ...
      if ((READ_BIT(RCC->PLL1FRACR, RCC_PLL1FRACR_PLL1FRACN) >> RCC_PLL1FRACR_PLL1FRACN_Pos) !=
        ...
        /* Wait at least 2 CK_REF (PLL input source divided by M) period to make sure next latched
...
uint32_t HAL_RCC_GetPCLK1Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq() >>
          ((APBPrescTable[(RCC->CFGR2 & RCC_CFGR2_PPRE1) >> RCC_CFGR2_PPRE1_Pos]) & 0x1FU));
}
...
uint32_t HAL_RCC_GetPCLK2Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq() >>
          ((APBPrescTable[(RCC->CFGR2 & RCC_CFGR2_PPRE2) >> RCC_CFGR2_PPRE2_Pos]) & 0x1FU));
}
...
uint32_t HAL_RCC_GetPCLK3Freq(void)
{
  ...
  return (HAL_RCC_GetHCLKFreq() >>
          ((APBPrescTable[(RCC->CFGR2 & RCC_CFGR2_PPRE3) >> RCC_CFGR2_PPRE3_Pos]) & 0x1FU));
}
/**
...
void HAL_RCC_GetClockConfig(RCC_ClkInitTypeDef *pClkInitStruct, uint32_t *pFLatency)
{
  ...
  pClkInitStruct->ClockType = RCC_CLOCKTYPE_SYSCLK | RCC_CLOCKTYPE_HCLK | RCC_CLOCKTYPE_PCLK1 |
