...
static HAL_StatusTypeDef FMAC_FilterPreload(FMAC_HandleTypeDef *hfmac, int16_t *pInput,
                                            uint8_t InputSize, int16_t *pOutput, uint8_t OutputSize,
                                            uint8_t PreloadAccess);
static void FMAC_WritePreloadDataIncrementPtr(FMAC_HandleTypeDef *hfmac, int16_t **ppData,
                                              uint8_t Size);
static HAL_StatusTypeDef FMAC_WaitOnStartUntilTimeout(FMAC_HandleTypeDef *hfmac, uint32_t Tickstart,
                                                      uint32_t Timeout);
static HAL_StatusTypeDef FMAC_AppendFilterDataUpdateState(FMAC_HandleTypeDef *hfmac,
                                                          int16_t *pInput, uint16_t *pInputSize);
 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_FilterPreload(FMAC_HandleTypeDef *hfmac, int16_t *pInput,
                                         uint8_t InputSize, int16_t *pOutput, uint8_t OutputSize)
{
  return (
      FMAC_FilterPreload(hfmac, pInput, InputSize, pOutput, OutputSize, PRELOAD_ACCESS_POLLING));
}
 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_FilterPreload_DMA(FMAC_HandleTypeDef *hfmac, int16_t *pInput,
                                             uint8_t InputSize, int16_t *pOutput,
                                             uint8_t OutputSize)
{
 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_FilterStart(FMAC_HandleTypeDef *hfmac, int16_t *pOutput,
                                       uint16_t *pOutputSize)
{
  uint32_t tmpcr = 0U;
  HAL_StatusTypeDef status;
  ...
  if (hfmac->State == HAL_FMAC_STATE_READY) {
    ...
    MODIFY_REG(hfmac->Instance->CR, FMAC_IT_RIEN | FMAC_IT_WIEN | FMAC_DMA_REN | FMAC_CR_DMAWEN,
               tmpcr);
    ...
    if (status == HAL_OK) {
      /* PARAM: Start the filter ( this can generate interrupts before the end of the
 ...
 * @retval HAL_StatusTypeDef HAL status
 */
HAL_StatusTypeDef HAL_FMAC_AppendFilterData(FMAC_HandleTypeDef *hfmac, int16_t *pInput,
                                            uint16_t *pInputSize)
{
  HAL_StatusTypeDef status;
 ...
 */
static HAL_StatusTypeDef FMAC_FilterConfig(FMAC_HandleTypeDef *hfmac,
                                           FMAC_FilterConfigTypeDef *pConfig, uint8_t PreloadAccess)
{
  uint32_t tickstart;
  uint32_t tmpcr;
  HAL_StatusTypeDef status;
  ...
  if (pConfig->InputBufferSize != 0U) {
    MODIFY_REG(hfmac->Instance->X1BUFCFG, (FMAC_X1BUFCFG_X1_BASE | FMAC_X1BUFCFG_X1_BUF_SIZE),
               (((((uint32_t) (pConfig->InputBaseAddress)) << FMAC_X1BUFCFG_X1_BASE_Pos) &
  ...
  if (pConfig->CoeffBufferSize != 0U) {
    MODIFY_REG(hfmac->Instance->X2BUFCFG, (FMAC_X2BUFCFG_X2_BASE | FMAC_X2BUFCFG_X2_BUF_SIZE),
               (((((uint32_t) (pConfig->CoeffBaseAddress)) << FMAC_X2BUFCFG_X2_BASE_Pos) &
  ...
  if (pConfig->OutputBufferSize != 0U) {
    MODIFY_REG(hfmac->Instance->YBUFCFG, (FMAC_YBUFCFG_Y_BASE | FMAC_YBUFCFG_Y_BUF_SIZE),
               (((((uint32_t) (pConfig->OutputBaseAddress)) << FMAC_YBUFCFG_Y_BASE_Pos) &
                 FMAC_YBUFCFG_Y_BASE) |
                                                          ...
                                                          int16_t *pInput, uint16_t *pInputSize)
{
  ...
  if (hfmac->InputAccess == FMAC_BUFFER_ACCESS_DMA) {
    ...
    if ((hfmac->hdmaIn->Mode & DMA_LINKEDLIST) == DMA_LINKEDLIST) {
      ...
      status =
          HAL_DMA_Start_IT(hfmac->hdmaIn, (uint32_t) pInput, (uint32_t) &hfmac->Instance->WDATA,
