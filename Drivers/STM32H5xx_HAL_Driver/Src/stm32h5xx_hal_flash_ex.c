    ...
    if ((pEraseInit->TypeErase & (~FLASH_NON_SECURE_MASK)) == FLASH_TYPEERASE_MASSERASE) {
      ...
      *SectorError = 0xFFFFFFFFU;
      ...
      for (sector_index = pEraseInit->Sector;
           sector_index < (pEraseInit->NbSectors + pEraseInit->Sector); sector_index++) {
...
#if defined(FLASH_SR_OBKERR)
    (*reg_cr) |= (FLASH_IT_EOP | FLASH_IT_WRPERR | FLASH_IT_PGSERR | FLASH_IT_STRBERR |
...
HAL_StatusTypeDef HAL_FLASHEx_ConfigHDPExtension(const FLASH_HDPExtensionTypeDef *pHDPExtension)
{
  ...
  if (pHDPExtension->Banks == FLASH_BANK_1) {
    ...
    FLASH->HDPEXTR =
        (pHDPExtension->NbSectors << FLASH_HDPEXTR_HDP2_EXT_Pos) | pHDPExtension->NbSectors;
  }
...
void HAL_FLASHEx_GetEccInfo(FLASH_EccInfoTypeDef *pData)
{
  ...
  if (addr_reg != 0xFFFFFFFFU) {
    ...
    case FLASH_ECC_AREA_EDATA:
      ...
      if ((correction_reg & FLASH_ECCR_BK_ECC) == FLASH_ECCR_BK_ECC) {
        /*
         * addr_reg is the address returned by the ECC register along with an offset value depends
         * on area To calculate the exact address set by user while an ECC occurred, we must
         * subtract the offset value, In addition, the address returned by ECC registers represents
         * 128-bit flash word (multiply by 4),
         */
        pData->Address =
            FLASH_EDATA_BASE + FLASH_BANK_SIZE + ((addr_reg - FLASH_ADDRESS_OFFSET_EDATA) * 4U);
      } else {
