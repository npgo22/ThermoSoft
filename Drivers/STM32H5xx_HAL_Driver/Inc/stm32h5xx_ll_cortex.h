...
#define LL_MPU_CTRL_HFNMI_PRIVDEF_NONE                                                             \
  0U /*!< Background region access not allowed, MPU disabled for Hardfaults, NMIs, and exception   \
        handlers when FAULTMASK=1 */
#define LL_MPU_CTRL_HARDFAULT_NMI                                                                  \
  2U /*!< Background region access not allowed, MPU enabled for Hardfaults, NMIs, and exception    \
        handlers when FAULTMASK=1 */
#define LL_MPU_CTRL_PRIVILEGED_DEFAULT                                                             \
  4U /*!< Background region privileged-only access allowed, MPU disabled for Hardfaults, NMIs, and \
        exception handlers when FAULTMASK=1 */
#define LL_MPU_CTRL_HFNMI_PRIVDEF                                                                  \
  6U /*!< Background region privileged-only access allowed, MPU enabled for Hardfaults, NMIs, and  \
        exception handlers when FAULTMASK=1 */
/**
 * @}
 */
...
{
  return ((READ_BIT(SysTick->CTRL, SysTick_CTRL_TICKINT_Msk) == (SysTick_CTRL_TICKINT_Msk)) ? 1UL
                                                                                            : 0UL);
}
 ...
 * @retval None
 */
__STATIC_INLINE void LL_MPU_ConfigRegionAddress(uint32_t Region, uint32_t BaseAddress,
 ...
 * @retval None
 */
__STATIC_INLINE void LL_MPU_ConfigRegionAddress_NS(uint32_t Region, uint32_t BaseAddress,
                                                   uint32_t LimitAddress)
{
  /* Set Region number */
  WRITE_REG(MPU_NS->RNR, Region);
