...
typedef struct {
  ...
  uint32_t
      OutputBurstType; /*!< Selects the output burst type.
...
{
  return ((READ_BIT(*((__IO uint32_t *) (&(ICACHE->CRR0) + (1U * Region))), ICACHE_CRRx_REN) ==
           (ICACHE_CRRx_REN))
              ? 1UL
              : 0UL);
}
...
void LL_ICACHE_ConfigRegion(uint32_t Region,
