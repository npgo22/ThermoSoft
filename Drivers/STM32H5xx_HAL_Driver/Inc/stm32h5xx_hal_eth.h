...
typedef struct {
  ...
  FunctionalState ChecksumOffload; /*!< Enables or Disable the checksum checking for received packet
                                      payloads TCP, UDP or ICMP headers */
  ...
  uint32_t PreambleLength; /*!< Selects or not the Preamble Length for Transmit packets (Full Duplex
                              mode). This parameter can be a value of @ref ETH_Preamble_Length */

  FunctionalState UnicastSlowProtocolPacketDetect; /*!< Enable or disables the Detection of Slow
  ...
  uint32_t GiantPacketSizeLimit; /*!< Specifies the packet size that the MAC will declare it as
                                    Giant, If it's size is greater than the value programmed in this
                                    field in units of bytes This parameter must be a number between
  ...
  uint32_t PauseLowThreshold; /*!< This field configures the threshold of the PAUSE to be checked
                                 for automatic retransmission of PAUSE Packet. This parameter can be
                                 a value of @ref ETH_Pause_Low_Threshold */

  FunctionalState
      TransmitFlowControl; /*!< Enables or disables the MAC to transmit Pause packets in Full Duplex
                              mode or the MAC back pressure operation in Half Duplex mode */

  FunctionalState UnicastPausePacketDetect; /*!< Enables or disables the MAC to detect Pause packets
  ...
  FunctionalState DropTCPIPChecksumErrorPacket; /*!< Enables or disables Dropping of TCPIP Checksum
                                                   Error Packets. */
...
typedef struct {
  ...
  uint32_t TxDMABurstLength; /*!< Indicates the maximum number of beats to be transferred in one Tx
                                DMA transaction. This parameter can be a value of @ref
                                ETH_Tx_DMA_Burst_Length */

  FunctionalState
      SecondPacketOperate; /*!< Enables or disables the Operate on second Packet mode, which allows
                              the DMA to process a second Packet of Transmit data even before
                                                      obtaining the status for the first one. */

  uint32_t RxDMABurstLength; /*!< Indicates the maximum number of beats to be transferred in one Rx
                                DMA transaction. This parameter can be a value of @ref
  ...
  uint32_t
      MaximumSegmentSize; /*!< Sets the maximum segment size that should be used while segmenting
                             the packet This parameter can be a value from 0x40 to 0x3FFF */
...
typedef struct {
  ...
  uint32_t TimestampChecksumCorrection; /*!< Enable checksum correction during OST for PTP over
                                           UDP/IPv4 packets */
...
typedef struct {
  ...
  uint32_t
      ControlPacketsFilter; /*!< Set the control packets filter
...
#define ETH_DMATXNDESCWBF_ES                                                                       \
  0x00008000U /*!< Error summary: OR of the following bits: IHE || UF || ED || EC || LCO || PCE || \
                 NC || LCA || FF || JT */
...
#define ETH_DMARXNDESCWBF_L3L4FM                                                                   \
  0x20000000U /*!< L3 and L4 Filter Number Matched: if reset filter 0 is matched , if set filter 1 \
                 is matched */
