...
#define FLASH_FLAG_SR_ERRORS                                                                       \
  (FLASH_SR_WRPERR | FLASH_SR_PGSERR | FLASH_SR_STRBERR | FLASH_SR_INCERR | FLASH_SR_OBKERR |      \
   FLASH_SR_OBKWERR | FLASH_SR_OPTCHANGEERR)
#else
#define FLASH_FLAG_SR_ERRORS                                                                       \
  ...
  do {                                                                                             \
    ...
    if (((__FLAG__) & ~(FLASH_FLAG_ECCR_ERRORS | FLASH_FLAG_OPTCHANGEERR)) != 0U) {                \
      WRITE_REG(FLASH->SECCCR,                                                                     \
                ((__FLAG__) & ~(FLASH_FLAG_ECCR_ERRORS | FLASH_FLAG_OPTCHANGEERR)));               \
    }                                                                                              \
  } while (0)
...
HAL_StatusTypeDef HAL_FLASH_Program_IT(uint32_t TypeProgram, uint32_t FlashAddress,
                                       uint32_t DataAddress);
...
#define IS_FLASH_TYPEPROGRAM(VALUE)                                                                \
  (((VALUE) == FLASH_TYPEPROGRAM_QUADWORD) || ((VALUE) == FLASH_TYPEPROGRAM_QUADWORD_NS) ||        \
   ...
   ((VALUE) == FLASH_TYPEPROGRAM_WORD_EDATA) || ((VALUE) == FLASH_TYPEPROGRAM_WORD_EDATA_NS) ||    \
...
#define IS_FLASH_OTP_ADDRESS(ADDRESS)                                                              \
  (((ADDRESS) >= FLASH_OTP_BASE) && ((ADDRESS) < (FLASH_OTP_BASE + FLASH_OTP_SIZE)))
